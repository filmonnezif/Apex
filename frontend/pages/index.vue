  ...
  <div class="space-y-6">
    ...
    <div v-if="analytics" class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div class="metric-card">
        <div class="text-blue-600 text-sm">Total Products</div>
        <div class="text-3xl font-bold text-blue-900">{{ analytics.total_products }}</div>
        <div class="text-blue-500 text-sm">Active SKUs</div>
      </div>
      <div class="metric-card">
        <div class="text-blue-600 text-sm">Estimated Revenue</div>
        <div class="text-3xl font-bold text-blue-900">AED {{ formatNumber(analytics.total_revenue_estimate) }}</div>
        <div class="text-green-600 text-sm">Monthly projection</div>
      </div>
      <div class="metric-card">
        <div class="text-blue-600 text-sm">Optimization Potential</div>
        <div class="text-3xl font-bold text-blue-900">+15%</div>
        <div class="text-yellow-600 text-sm">Revenue increase</div>
      </div>
    </div>
    ...
    <div class="glass-card p-6">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-blue-900">Product Portfolio</h2>
        <button @click="refreshData" class="glass-button-secondary">
      ...
      <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div v-for="i in 6" :key="i" class="glass-card p-6 shimmer">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          <!-- Product Image -->
          <div class="w-full h-48 mb-4 rounded-lg overflow-hidden bg-gradient-to-br from-blue-50 to-cyan-50 flex items-center justify-center">
            <ProductImage 
          ...
          <div class="flex items-start justify-between mb-3">
            <div>
              ...
              <h3 class="text-blue-900 font-semibold text-lg">{{ product.name }}</h3>
            </div>
          ...
          <div class="space-y-2 mb-4">
            ...
            <div class="flex justify-between text-sm">
              ...
              <span class="text-blue-700">{{ product.category }}</span>
            </div>
          </div>
